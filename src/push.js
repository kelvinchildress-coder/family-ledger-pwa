...
export async function sendPushSubscriptionToServer(u,sub,url){if(!sub||!url)return;try{await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'subscribe',userId:u,userName:u,subscription:sub})})}catch(e){console.error(e)}}
